...
Scraper discoveries preserve source query, source provider, Google evidence, normalized identity fields, official phone verification status/source/date, owner evidence, separate scores, score reasons, duplicate warnings, and review outcome. Approved/rejected review records are not overwritten by later reruns. Marketing weaknesses are stored as separate evidence rows. Scoring policies are organization-scoped and editable by Founder permission. Domain rate-limit rows track official-site fetch timing.
